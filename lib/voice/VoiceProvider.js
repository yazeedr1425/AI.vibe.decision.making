...
export function VoiceProvider({ children }) {
  ...
  // ضبط مباشر — تستخدمه صفحة الإعدادات ومزامنة البروفايل
  const setReadAloud = useCallback((next) => {
    if (next === readAloudState) return;
    if (!next) stopSpeaking();
    writeReadAloud(Boolean(next));
  }, []);
  ...
  const value = useMemo(
    ...
    [readAloud, toggleReadAloud, setReadAloud, announce, repeat, tts, stt],
  );
