...
import { budget, formatClock, group, isOutdoor, vibe } from "./config";
...
5. Weather — applies ONLY when an hourly forecast appears below. Candidates tagged "outdoor" are exposed to the weather; untagged ones are sheltered. Put outdoor stops in the coolest hours of the window and keep them out of the peak-heat hours. If the whole window is hot, or rain probability is high, lean toward untagged (indoor) stops and keep any outdoor stop short.
...
- When weather drove a placement, work the reason into "why" as a short natural clause — "قبل ما تشتد الشمس", "بعد ما يبرد الجو", "والجو يسمح". One clause woven into the sentence. Never a forecast readout, never numbers, and not on every stop — only where it actually mattered.
...
function describePlace(place, index) {
  ...
  // الوسم الذي تتعلّق به قاعدة الطقس. غيابه لا يعني "مغلق" بل
  // "غير مؤكد"، والقاعدة مصاغة على هذا الأساس.
  if (isOutdoor(place.category)) bits.push("outdoor");
...
// كتلة الطقس تدخل البرومبت فقط لما يرجع التوقّع فعلاً. غيابها يعني
// أن القاعدة الخامسة تبقى بلا مدخلات، وهي مصاغة لتكون بلا أثر حينها.
function describeWeather(weather) {
  if (!weather) return [];

  const unit = weather.unit;
  const rows = weather.hours.map((h) => {
    const rain = h.rain != null ? ` · rain ${h.rain}%` : "";
    return `  ${h.time} · feels ${h.feels}${unit}${rain}`;
  });

  return [
    "",
    "Hourly forecast for the window (feels-like, local time):",
    ...rows,
    `Hottest ${weather.hottest.time} (${weather.hottest.feels}${unit}). ` +
      `Coolest ${weather.coolest.time} (${weather.coolest.feels}${unit}).` +
      (weather.maxRain != null
        ? ` Peak rain probability ${weather.maxRain}%.`
        : ""),
  ];
}
