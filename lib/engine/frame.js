...
import { sliceOf } from "../time/now.js";
...
export const FRAME_SYSTEM =
  ...
  "12. You may be told the user's current local time. Use it ONLY where it genuinely " +
  "changes which option wins — a late hour, a weekend, a working morning. Where it " +
  "changes nothing, ignore it entirely rather than mentioning it. Never state a fact " +
  "about that time you were not given, and never infer opening hours, crowding, " +
  "weather or travel time from it — you were given a clock reading, nothing more.";

// أسماء الأيام والشرائح بالعربية: النموذج يقرأ سطراً عربياً وسط
// برومبت عربي، والقيم الإنجليزية تبقى للآلة (المفتاح والمقارنة).
const WEEKDAY_AR = {
  sunday: "الأحد",
  monday: "الاثنين",
  tuesday: "الثلاثاء",
...
export function framePrompt(options, now = null) {
  ...
  // سطر حقائق لا تعليمات: الساعة معطاة، والقاعدة ١٢ تقرر متى تُستعمل.
  // يسقط السطر كله حين لا يُعرف الوقت بدل ما يُرسل فارغاً.
  const when = now
    ? [
        `الوقت عند المستخدم: ${WEEKDAY_AR[now.weekday]} الساعة ` +
          `${toArabicDigits(now.hour)} ${SLICE_AR[sliceOf(now)]}` +
          `${now.isWeekend ? " (عطلة نهاية الأسبوع)" : ""}.`,
        "",
      ]
    : [];
...
export const REFINE_SYSTEM =
  ...
  "6. Short Saudi-dialect Arabic, Arabic-Indic digits, no decorative phrasing.";
