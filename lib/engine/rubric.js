...
export function validateRubric(raw, base) {
  ...
  for (const c of raw.criteria) {
    ...
    const low = text(c?.low, 40);
    const high = text(c?.high, 40);
    // low/high هما طرفا المقياس في RatingGrid — بدونهما الشبكة
    // تعرض عمودين بلا عنوان ويصير التقييم تخميناً
    if (!label || !low || !high) return fail("CRITERION_TEXT", key);

    keys.add(key);
    criteria.push({ key, label, low, high });
  }
