...
// يُرفع يدوياً كل ما تغيّر شكل المخرجات أو البرومبت.
//
// بدونه تبقى المدخلات القديمة صالحة بعد أي تغيير في العقد، فتُخدَم
// بشكل ناقص: لما أضفنا mid لأزرار التقييم رجّع الكاش نسخاً بلا هذا
// الحقل، والنتيجة أزرار "undefined" على شاشة المستخدم بينما الكود
// الجديد سليم تماماً. تغيير المفتاح يجعل القديم غير قابل للإصابة.
const RUBRIC_VERSION = "v2-scale";
...
export function rubricKey({ options, categoryId }) {
  return [RUBRIC_VERSION, categoryId, ...options.map(normalize).sort()].join("|");
}
