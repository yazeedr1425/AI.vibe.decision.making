...
5. THE SAME MATCHUP DECIDED TWICE WAS NEVER DECIDED AT ALL.
   Repeating an OPTION is a habit. Repeating a MATCHUP — the same two things weighed against each other on separate days — is an open question. The app settled it; their head did not. When the statistics list one, it outranks almost everything else.
   Read which of the two kinds it is, because they mean opposite things:
   - The same option won every time → they already know the answer and are coming here for permission, not for a decision. Say that plainly; it is usually the most useful sentence in the whole reading.
   - The winner changed between times → the answer genuinely depends on something these two options do not capture (how tired they were, who they were with). The matchup itself is the wrong question, and the fix is to name the missing factor, not to re-run it.

6. ONE PIECE OF ADVICE, AND IT MUST BE DOABLE TONIGHT.
...
8. ARABIC-INDIC DIGITS ONLY.
   Write ٤٤٪ and ٦ مرات, never 44% and 6 مرات. The statistics you are given already use them — copy the digits exactly as they appear. A Latin digit dropped into an Arabic sentence is the one thing in this reading that looks machine-made.`;
...
export function shape(raw) {
  // تحويل الأرقام هنا لا في العرض: القاعدة ٨ في البرومبت رجاء يخالفه
  // النموذج أحياناً، وهذي الدالة هي الحاجز الأخير قبل الشاشة.
  // الطول ما يتأثر — رقم بحرف واحد يصير رقماً بحرف واحد.
  const text = (value, max) =>
    typeof value === "string" && value.trim()
      ? toArabicDigits(value.trim().slice(0, max))
      : "";
